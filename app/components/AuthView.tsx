...
export default function AuthView() {
  const { registerUser, loginUser } = useMultiplayer();
  ...
  return (
    <main className="min-h-screen bg-[#FDFBF7] p-6 pb-24 font-sans max-w-md mx-auto flex flex-col justify-center">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8">
            ...
            <div className="flex flex-wrap gap-2">
              {AVATARS.map((a) => (
                <button
                  ...
                  className={`text-3xl w-12 h-12 flex items-center justify-center rounded-2xl transition-all ${
                    selectedAvatar === a
                      ? "bg-amber-200 ring-4 ring-amber-400 scale-110"
        ...
        >
          {isLoading ? "Loading..." : isLoginMode ? "Log In" : "Create Account"}
        </button>
      </form>
